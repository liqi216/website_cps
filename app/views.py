...
class GuestProcessView(ModelView):
    ...
    label_columns = {'guest_pro_name': 'Scenario Name','process_name':'Scenario Name','process_description':'Description','process_public':'Public'}
...
class GuestProcessCmpView(ModelView):
    ...
    label_columns = {'process_name': 'Scenario Name','process_description':'Description','process_public':'Public'}
...
class ProcessView(CopyModelView):
    ...
    label_columns = {'pro_name': 'Scenario Name','process_name':'Scenario Name','process_description':'Description','process_public':'Public'}
    add_columns =  ['process_name','process_description', 'simple_scenario']
    edit_columns =  ['process_name','process_description']
    list_columns = ['pro_name','created_by', 'created_on', 'changed_by', 'changed_on','is_public','simple_scenario']
    formatters_columns = {'created_on': lambda x: x.strftime("%Y-%m-%d %H:%M:%S"),'changed_on': lambda x: x.strftime("%Y-%m-%d %H:%M:%S") }
    ...
    def pre_delete(self, item):
        if item.created_by.id != current_user.id and current_user.roles[0].name != 'Admin':
...
class AdvancedMseView(ModelView):
    ...
    label_columns = {'process_name' : 'Scenario Name','process_description':'Description','process_public':'Public'}
    list_columns = ['advance_compare','process_name','created_by', 'created_on', 'changed_by', 'changed_on','simple_scenario']
...
class ProStepView(BaseView):
    ...
    def trackChanges(self, pk):
        ...
        if Decimal(pgi.louisiana) != global_settings.louisiana:
            changes.add("louisiana")
        if Decimal(pgi.mississippi) != global_settings.mississippi:
            changes.add("mississippi")
        if Decimal(pgi.alabama) != global_settings.alabama:
            changes.add("alabama")
        if Decimal(pgi.texas) != global_settings.texas:
            changes.add("texas")
        if Decimal(pgi.florida) != global_settings.florida:
            changes.add("florida")
