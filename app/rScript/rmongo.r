...
function(process_gen_id){
  ...
  act_mrip_e_retention_len<-mrip_e_retention_len*(1+exp(-(as.numeric(rownames(length_age_key))-ref_Comme_min_size_cm)))/(1+exp(-(as.numeric(rownames(length_age_key))-Comme_min_size_cm)))
  ...
  act_mrip_w_retention_len<-mrip_w_retention_len*(1+exp(-(as.numeric(rownames(length_age_key))-ref_Comme_min_size_cm)))/(1+exp(-(as.numeric(rownames(length_age_key))-Comme_min_size_cm)))
  ...
  act_hbt_e_retention_len<-hbt_e_retention_len*(1+exp(-(as.numeric(rownames(length_age_key))-ref_Comme_min_size_cm)))/(1+exp(-(as.numeric(rownames(length_age_key))-Comme_min_size_cm)))
  ...
  act_hbt_w_retention_len<-hbt_w_retention_len*(1+exp(-(as.numeric(rownames(length_age_key))-ref_Comme_min_size_cm)))/(1+exp(-(as.numeric(rownames(length_age_key))-Comme_min_size_cm)))
  ...
  Function_relF_imple<-function(v){
    F1<-v[1]
    F2<-v[2]
    abs(Comme_planned_catch-sum(true_N_1*(1-exp(-F1*(comp_comm_sel_e+comp_recr_sel_e + comp_comm_discard_e+comp_recr_discard_e) - comp_bycatch_e - M_1_true))*
                          F1*comp_comm_sel_e/(F1*(comp_comm_sel_e+comp_recr_sel_e + comp_comm_discard_e+comp_recr_discard_e) + comp_bycatch_e + M_1_true) * weight_at_age_1 + 
                                true_N_2*(1-exp(-F1*(comp_comm_sel_w+comp_recr_sel_w + comp_comm_discard_w+comp_recr_discard_w) - comp_bycatch_w - M_2_true))*
                          F1*comp_comm_sel_w/(F1*(comp_comm_sel_w+comp_recr_sel_w + comp_comm_discard_w+comp_recr_discard_w) + comp_bycatch_w + M_2_true) * weight_at_age_2))
    +abs(Recre_planned_catch-sum(true_N_1*(1-exp(-F2*(comp_comm_sel_e+comp_recr_sel_e + comp_comm_discard_e+comp_recr_discard_e) - comp_bycatch_e - M_1_true))*
                          F2*comp_recr_sel_e/(F2*(comp_comm_sel_e+comp_recr_sel_e + comp_comm_discard_e+comp_recr_discard_e) + comp_bycatch_e + M_1_true) * weight_at_age_1 +
                                true_N_2*(1-exp(-F2*(comp_comm_sel_w+comp_recr_sel_w + comp_comm_discard_w+comp_recr_discard_w) - comp_bycatch_w - M_2_true))*
                          F2*comp_recr_sel_w/(F2*(comp_comm_sel_w+comp_recr_sel_w + comp_comm_discard_w+comp_recr_discard_w) + comp_bycatch_w + M_2_true) * weight_at_age_2))
  }
  ...
  for (i.run in 1:Simrun_Num){
    ...
    for (i.runtime in 1:Runtime_short){
      ...
      total_F_1<-relF * comp_recr_sel_e + relF * comp_comm_sel_e + 
        relF * comp_recr_discard_e + relF * comp_comm_discard_e + 
        relF * rec_closed_e_pred_F_ave * rec_closed_e_selex * Recre_discard_E_closed+ 
        relF * comm_closed_e_pred_F_ave * comm_closed_e_selex * Comme_discard_E_closed +
        relF * shrimp_e_pred_F_ave * shrimp_e_selex #retention 0 discard 1 for shirmp
      
      total_F_2<-relF * comp_recr_sel_w + relF * comp_comm_sel_w + 
        relF * comp_recr_discard_w + relF * comp_comm_discard_w + 
        relF * rec_closed_w_pred_F_ave * rec_closed_w_selex * Recre_discard_W_closed + 
        relF * comm_closed_w_pred_F_ave * comm_closed_w_selex * Comme_discard_W_closed +
        relF * shrimp_w_pred_F_ave * shrimp_w_selex 
  ...
  for (i.run in 1:Simrun_Num){
    ...
    for (i.runtime in 1:Runtime_long){
      ...
      total_F_1_imple<-recr_relF_imple * comp_recr_sel_e + comm_relF_imple * comp_comm_sel_e + 
        recr_relF_imple * comp_recr_discard_e + comm_relF_imple * comp_comm_discard_e + comp_bycatch_e

      total_F_2_imple<-recr_relF_imple * comp_recr_sel_w + comm_relF_imple * comp_comm_sel_w + 
        recr_relF_imple * comp_recr_discard_w + comm_relF_imple * comp_comm_discard_w + comp_bycatch_w
